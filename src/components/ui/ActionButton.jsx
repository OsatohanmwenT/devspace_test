const variants = {
  ...
  neutral: 'rounded-[14px] text-[var(--text-primary)] shadow-[0_4px_0_var(--border-hairline),inset_0_1px_0_rgba(255,255,255,.08)] hover:brightness-110 hover:-translate-y-0.5 active:translate-y-1 active:shadow-[inset_0_1px_0_rgba(255,255,255,.08)]',
}

export function ActionButton({ variant = 'primary', className = '', children, ...props }) {
  return (
    <button
      className={`relative border-0 px-4 transition-[transform,filter,box-shadow] duration-75 focus-visible:outline-3 focus-visible:outline-[var(--action-blue-focus)] focus-visible:outline-offset-4 ${variants[variant]} ${className}`}
      {...props}
    >
      {children}
    </button>
