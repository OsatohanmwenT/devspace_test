    ...
    <button type="button" aria-label={'Open ' + currentPath.title} onClick={onOpenDetail} className="flex h-full min-h-[164px] w-full flex-col items-start justify-between gap-5 max-[680px]:min-h-0 max-[680px]:gap-4 rounded-2xl border border-[#404040] bg-[#1f1f1f] px-5 py-4 text-left transition-[border-color,background,transform] duration-150 hover:-translate-y-0.5 hover:border-[#525252] hover:bg-[#252525] focus-visible:outline-3 focus-visible:outline-[#93c5fd] focus-visible:outline-offset-3 [[data-theme=light]_&]:border-[#d4d4d4] [[data-theme=light]_&]:bg-white [[data-theme=light]_&]:shadow-[0_2px_6px_rgba(20,20,20,0.06)] [[data-theme=light]_&]:hover:bg-[#fafafa]">
      ...
      <span className="flex w-full items-center gap-3">
        <span className="relative h-2 flex-1 overflow-hidden rounded-full bg-[#333336] [[data-theme=light]_&]:bg-[#ececea]" aria-hidden="true">
          <span className="absolute inset-y-0 left-0 rounded-full bg-[#16a34a]" style={{ width: `${currentPath.progressValue ?? 0}%` }} />
        </span>
        <span className="flex-none rounded-lg bg-[#16a34a] px-3 py-2 text-[13px] font-semibold text-white">Open path</span>
      </span>
    </button>
  )
}
