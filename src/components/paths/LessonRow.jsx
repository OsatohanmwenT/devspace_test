...
export function LessonRow({ lesson, index, isSelected, onSelect }) {
  ...
  return (
    <div className={`relative z-[1] flex min-h-[126px] w-full ${alignment}`} data-current-lesson={isCurrent || undefined}>
      <button
        type="button"
        className="group flex min-h-[126px] w-[min(100%,278px)] items-center gap-0 rounded-[18px] border-0 bg-transparent px-2 text-left text-[#f4f4f2] disabled:cursor-not-allowed focus-visible:outline-3 focus-visible:outline-[#9db5d7] focus-visible:outline-offset-3 [[data-theme=light]_&]:text-neutral-800"
        disabled={isLocked}
      ...
      >
        ...
        <span className={`grid min-w-0 gap-[5px] ${isCurrent ? '-ml-6' : ''}`}>
