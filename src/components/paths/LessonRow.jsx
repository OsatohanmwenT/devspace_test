...
export function LessonRow({ lesson, index, isSelected, onSelect }) {
  ...
  return (
    <div className={`relative z-[1] flex min-h-[126px] w-full ${alignment}`}>
      <button
        type="button"
        className="group flex min-h-[126px] w-[min(100%,278px)] items-center gap-2.5 rounded-[18px] border-0 bg-transparent px-2 text-left text-[#f4f4f2] disabled:cursor-not-allowed focus-visible:outline-3 focus-visible:outline-[#9f9dd7] focus-visible:outline-offset-3 [[data-theme=light]_&]:text-[#202020]"
        disabled={isLocked}
      ...
      >
        ...
        <span className="grid min-w-0 gap-[5px]">
          ...
          <strong className="text-[15px] leading-[1.35] font-medium text-[#f4f4f2] [[data-theme=light]_&]:text-[#202020]">{lesson.title}</strong>
