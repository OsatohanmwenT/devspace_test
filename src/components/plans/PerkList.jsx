    ...
    <ul className="grid gap-2.5 m-0 p-0 list-none">
      {perks.map((perk) => (
        <li
          key={perk.id}
          id={`perk-${perk.id}`}
          className={`flex items-start gap-2.5 rounded-xl p-2.5 ${perk.id === highlightPerk ? 'bg-[rgba(102, 153, 236,0.14)] ring-1 ring-[#6699ec]' : ''}`}
        >
          <CheckIcon className="w-4 h-4 mt-0.5 flex-none text-[#04adc0]" />
          <div className="grid gap-0.5">
            <strong className="text-sm font-semibold text-[#f4f4f2] [[data-theme=light]_&]:text-neutral-800">{perk.title}</strong>
