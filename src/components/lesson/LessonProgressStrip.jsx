    ...
    >
      ...
      >
        {Array.from({ length: totalSteps }, (_, index) => (
          <span
            className={index < currentStep ? 'h-2 rounded bg-[#2986f2]' : 'h-2 rounded bg-[#404040] [[data-theme=light]_&]:bg-[#e1e1e1]'}
            key={index}
          />
        ))}
      </div>
      <button type="button" className={navButtonClassName} disabled aria-label="Next lesson step">
