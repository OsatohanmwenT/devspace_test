...
const GRID = 'grid grid-cols-[28px_40px_minmax(0,1fr)_auto] max-[680px]:grid-cols-[22px_34px_minmax(0,1fr)_auto] items-center gap-3.5 max-[680px]:gap-2.5 p-3.5 max-[680px]:p-2.5 rounded-xl border'

export function LeaderboardRow({ entry, isCurrentUser }) {
  const tier = MEDALS[entry.rank]

  return (
    <div className={isCurrentUser ? `${GRID} border-[#6f66ec] bg-[#2a293c] [[data-theme=light]_&]:bg-[#f1f0fd]` : `${GRID} border-transparent`}>
      ...
      <span className="text-[#f4f4f2] [[data-theme=light]_&]:text-[#202020] text-sm tabular-nums whitespace-nowrap">{entry.score.toLocaleString()}</span>
    </div>
  )
}
