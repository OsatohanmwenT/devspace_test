...
// delta is null for the first day of the season, when there is no previous
// day to measure against — that is different from having held your place, so
// it reads as blank rather than as a dash. Never colors the whole row: only
// this cell moves, which is what section 8 means by "not the entire row".
function RankDelta({ delta }) {
  if (delta === null || delta === undefined) {
    return (
      <span className="text-center text-[11px] text-[#5a5a5e] [[data-theme=light]_&]:text-[#a8a8a4]" aria-hidden="true">
        —
      </span>
    )
  }
  ...
  if (delta === 0) {
    return (
      <span className="text-center text-[11px] tabular-nums text-[#7d7d80] [[data-theme=light]_&]:text-[#737371]" aria-hidden="true">
        —
      </span>
    )
  }
  ...
  return (
    <span className={`text-center text-[11px] font-semibold tabular-nums ${climbed ? 'text-[#04adc0] [[data-theme=light]_&]:text-[#065f6b]' : 'text-[#ff676d] [[data-theme=light]_&]:text-[#b3272d]'}`}>
...
export function LeaderboardRow({ entry, isCurrentUser, onSelect }) {
  ...
  return (
    <Wrapper
      ...
      className={`${isCurrentUser ? `${ROW_GRID} my-1 rounded-2xl border border-[#8bb5f5] bg-[#2a293c] [[data-theme=light]_&]:border-[#b9d4f7] [[data-theme=light]_&]:bg-[#e9f2ff]` : `${ROW_GRID} rounded-xl border border-transparent transition-colors duration-150 hover:bg-[#262626] [[data-theme=light]_&]:hover:bg-[#f3f1ec]`} ${onSelect ? 'w-full cursor-pointer text-left font-[inherit]' : ''}`}
    >
      ...
      <Avatar name={entry.name} avatarStyle={entry.avatarStyle} avatarSeed={entry.id} size="md" />
      ...
      <div className="grid justify-items-end gap-0.5">
        <span className={`text-[#f4f4f2] [[data-theme=light]_&]:text-neutral-800 text-sm tabular-nums whitespace-nowrap ${isCurrentUser ? 'font-semibold' : ''}`}>
          {entry.score.toLocaleString()} 🪙
        </span>
        {entry.reward > 0 && (
          <span className="text-[11px] font-semibold tabular-nums text-[#04adc0] [[data-theme=light]_&]:text-[#065f6b]">
