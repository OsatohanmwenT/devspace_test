...
function HiddenRows({ count, locked, onExpand }) {
  return (
    <button
      type="button"
      onClick={onExpand}
      className="w-full py-2 text-[11px] font-semibold tracking-[.06em] uppercase text-[var(--text-muted)] [[data-theme=light]_&]:text-[var(--text-secondary)] hover:text-[var(--text-primary)] [[data-theme=light]_&]:hover:text-[var(--text-primary)] focus-visible:rounded-lg focus-visible:outline focus-visible:outline-3 focus-visible:outline-[var(--border-focus)] [[data-theme=light]_&]:focus-visible:outline-[var(--brand-base)] focus-visible:outline-offset-2"
    >
  ...
  return (
    <div className="relative">
      ...
      <div className="relative mt-1 grid gap-2 overflow-hidden rounded-xl">
        ...
        <div className="absolute inset-0 grid place-items-center bg-[var(--surface-default)]/92">
          <div className="grid justify-items-center gap-2 text-center">
          <span className="text-sm font-medium text-[var(--text-primary)] [[data-theme=light]_&]:text-[var(--text-primary)]">See every XP you&apos;ve earned.</span>
          <button
            type="button"
            onClick={() => onOpenPlans?.('all-time')}
            className="rounded-full border border-[var(--border-default)] [[data-theme=light]_&]:border-[var(--border-hairline)] bg-[var(--surface-raised)] [[data-theme=light]_&]:bg-white px-4 py-2 text-sm font-medium text-[var(--text-primary)] [[data-theme=light]_&]:text-[var(--text-primary)] hover:brightness-110"
          >
...
export default function LeaderboardView({
  ...
  const progressStatus = summary?.inDemotion
    ? { label: `${summary.gapToSafety.toLocaleString()} px to safety`, tone: 'text-[var(--accent-error)] [[data-theme=light]_&]:text-[var(--accent-error)]' }
    : summary?.inPromotion
      ? { label: summary.promotionCushion > 0 ? `Promotion zone · ${summary.promotionCushion.toLocaleString()} px clear` : 'Promotion zone · hold your place', tone: 'text-[var(--accent-data)] [[data-theme=light]_&]:text-[var(--accent-data)]' }
      : summary?.promotesAnyone
        ? { label: `${summary.gapToPromotion.toLocaleString()} px to promotion`, tone: 'text-[var(--brand-on-dark)] [[data-theme=light]_&]:text-[var(--brand-cta)]' }
        : { label: 'Holding your place', tone: 'text-[var(--text-secondary)] [[data-theme=light]_&]:text-[var(--text-secondary)]' }
  ...
  return (
    <section className="grid gap-9" aria-label="Leaderboard">
      ...
      <div className="grid justify-items-center gap-3 text-center max-w-[640px] mx-auto">
        ...
        <h1 className="m-0 mt-1 text-[var(--text-primary)] [[data-theme=light]_&]:text-[var(--text-primary)] font-['Rethink_Sans',Arial,sans-serif] text-[clamp(26px,3.2vw,32px)] font-medium leading-tight">{league.name}</h1>
      ...
      {!hasJoined ? (
        ...
        <div className="grid w-full max-w-[680px] mx-auto gap-6">
          <div className="flex items-center justify-between gap-4 border-y border-[var(--border-default)] [[data-theme=light]_&]:border-[var(--border-hairline)] px-2 py-3" role="status">
            <div className="flex items-center gap-3">
              <span className="rounded-full border border-[var(--border-interactive)] [[data-theme=light]_&]:border-[var(--border-default)] px-2.5 py-1 text-[11px] font-bold tabular-nums text-[var(--text-primary)] [[data-theme=light]_&]:text-[var(--text-primary)]">#{summary.user.rank}</span>
              <div className="grid gap-0.5">
                <span className="text-[11px] font-bold uppercase tracking-[.08em] text-[var(--text-muted)] [[data-theme=light]_&]:text-[var(--text-secondary)]">Next target</span>
          ...
          {tab === 'All time' && !hasAllTime ? (
            ...
            <ol className="grid list-none m-0 p-0 border-y border-[var(--border-default)] [[data-theme=light]_&]:border-[var(--border-hairline)]" aria-label={`${league.name} standings`}>
              <li className="grid grid-cols-[minmax(0,1fr)_auto_44px] max-[680px]:grid-cols-[minmax(0,1fr)_auto_34px] gap-3.5 max-[680px]:gap-2.5 px-4 pt-3 pb-2 text-[10px] font-semibold tracking-[.08em] uppercase text-[var(--text-muted)] [[data-theme=light]_&]:text-[var(--text-secondary)]">
                <span>Learner</span>
                <span>px</span>
              ...
              {(tab === 'This week' ? rendered
                ...
                if (item.type === 'promotion-line') {
                  return <li key="promotion-line"><ZoneDivider label="Promotion zone ends" color="var(--accent-data)" /></li>
                }
                if (item.type === 'demotion-line') {
                  return <li key="demotion-line"><ZoneDivider label="Demotion zone starts" color="var(--accent-error)" /></li>
                }
                return (
                  <li key={item.entry.id} className="border-t border-[var(--border-default)] [[data-theme=light]_&]:border-[var(--border-hairline)]">
